      ...
      <>
        {
          ...
          <div key={key} className='col-span-1'>
            <Link href={"/habilidad/otros"}  className='text-center w-full py-3 rounded bg-white border-purplePrimary border-2 text-purpleSecondary' type='button' >
              {option}
            </Link>
          </div>
          ) : (
            <div key={key} className='col-span-1'>
            <button className='w-full py-3 rounded bg-white border-purplePrimary border-2 text-purpleSecondary' type='button' onClick={()=>{addSkill(option, pathname)}}>
              {option}
            </button>
          </div>
          )
